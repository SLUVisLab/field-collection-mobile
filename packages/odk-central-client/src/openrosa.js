 ...
 *
 * Note on `hash` semantics (verified live against Central v2026.2):
 * - For **static media** (`type: null`), `hash` is `md5:<md5-of-bytes>` — the
 *   delivered bytes verify against it.
 * - For an **`entityList`** resource, `hash` is a **dataset content signature**
 *   (a change-detection token, echoed as the download's `ETag`), **not** an md5
 *   of the delivered CSV bytes. Treat it as a version token (refetch when it
 *   changes); use `integrityUrl` for Entity-level integrity/deletion state.
 */
 ...
 *
 * Central distinguishes resource kinds at the protocol level via the `type`
 * attribute on `<mediaFile>` and an optional `<integrityUrl>`:
 *
 * ```xml
 * <manifest xmlns="http://openrosa.org/xforms/xformsManifest">
 *   <mediaFile type="entityList">            <!-- a linked Entity List / Dataset -->
 *     <filename>plants.csv</filename>
 *     <hash>md5:...</hash>
 *     <downloadUrl>https://.../attachments/plants.csv</downloadUrl>
 *     <integrityUrl>https://.../datasets/plants/integrity</integrityUrl>
 *   </mediaFile>
 *   <mediaFile>                               <!-- ordinary static form media -->
 ...
 * ```
 *
 * Each entry therefore exposes `type` (e.g. `"entityList"` for a linked Dataset,
 * `null` for plain media), the convenience flag `isEntityList`, and the optional
 * `integrityUrl`. The download path itself is identical for both kinds — callers
 * fetch `downloadUrl` with {@link OdkCentralClient.downloadFormAttachment} — so
 * no Entity-specific transport is required; the distinction is purely
 * descriptive metadata the caller may use.
 *
 * Behavior:
