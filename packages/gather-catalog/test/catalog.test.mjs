...
test('Gather catalog contract has stable versioned identifiers', () => {
  ...
  assert.equal(GATHER_ACTION_IDS.advance, 'gather.advance');
  assert.equal(GATHER_ACTION_IDS.back, 'gather.back');
  assert.ok(!('phaseView' in GATHER_COMPONENT_IDS), 'conditional-rendering component was removed');
