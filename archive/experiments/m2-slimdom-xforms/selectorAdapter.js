const ADAPTER_SYMBOL = Symbol.for('m2.slimdom.selectorAdapterInstalled');
...
const parseAttributeSelector = (input) => {
  ...
  const value =
    doubleQuotedValue ?? singleQuotedValue ?? unquotedValue ?? null;
...
const querySelectorAllFromContext = (contextNode, selectorText) => {
  ...
  results.sort((a, b) => compareDocumentOrder(contextNode, a, b));
  return results;
};
...
const matchesSelector = (element, selectorText) => {
  ...
  const matches = querySelectorAllFromContext(parentContext, selectorText);
...
const installSlimdomSelectorAdapter = ({ ElementClass, DocumentClass }) => {
  ...
  const rememberDescriptor = (target, methodName) => {
    previousDescriptors.set(
      `${target === ElementClass.prototype ? 'Element' : 'Document'}:${methodName}`,
      Object.getOwnPropertyDescriptor(target, methodName) ?? null
    );
  };

  rememberDescriptor(ElementClass.prototype, 'querySelectorAll');
  rememberDescriptor(ElementClass.prototype, 'querySelector');
  rememberDescriptor(ElementClass.prototype, 'matches');
  rememberDescriptor(DocumentClass.prototype, 'querySelectorAll');
  rememberDescriptor(DocumentClass.prototype, 'querySelector');
