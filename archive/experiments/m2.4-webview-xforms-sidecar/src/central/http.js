import { OdkCentralError, ODK_CENTRAL_ERROR_CODES, errorFromResponse, redactUrl } from './errors.js';
import { parseOpenRosaResponse } from './openrosa.js';
...
export const request = async ({
  ...
  try {
    ...
    if (isAbortError(error)) {
      throw new OdkCentralError(`Request timed out: ${method} ${redactUrl(requestUrl)}`, {
        code: ODK_CENTRAL_ERROR_CODES.TIMEOUT,
        cause: error,
      });
    }
    throw new OdkCentralError(`Network error: ${method} ${redactUrl(requestUrl)}`, {
  ...
  if (!response.ok) {
    ...
    // Central's OpenRosa endpoints return errors as an <OpenRosaResponse> XML
    // document; surface its <message> so callers get the real reason.
    const openRosaMessage =
      bodyJson == null && bodyText != null && bodyText.includes('OpenRosaResponse')
        ? parseOpenRosaResponse(bodyText).message
        : null;
    throw errorFromResponse(response, { bodyText, bodyJson, openRosaMessage });
  }
