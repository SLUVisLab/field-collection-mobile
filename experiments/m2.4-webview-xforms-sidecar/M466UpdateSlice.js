...
const ScenarioRunner = ({ pushEvent, setSummary }) => {
  ...
  useEffect(() => {
    ...
    const runStep = async (steps, name, op) => {
      console.log(`M466_RUNTIME_STEP_START::${name}`);
      pushEvent({ type: 'step-start', payload: { name } });
      try {
        const value = await op();
        steps.push({ name, ok: true });
        console.log(`M466_RUNTIME_STEP_OK::${name}`);
        return value;
      } catch (error) {
        const resolved = error instanceof Error ? error : new Error(String(error));
        steps.push({ name, ok: false, error: resolved.message });
        console.log(`M466_RUNTIME_STEP_FAIL::${name}::${resolved.message}`);
        throw resolved;
      }
    };
    ...
    const run = async () => {
      ...
      await runStep(steps, 'loadFormWithResources', () =>
        latestFormRef.current.loadForm(formXml, [
          { filename: 'plants.csv', contentType: 'text/csv', text: csvText },
        ])
      );
      await waitForTick();
      await waitForTick();

      // Pick an existing Entity and change its status to a different value.
      // Use a different row per platform so iOS and Android don't race on the
      // same Entity's version/baseVersion across back-to-back runs.
      const rows = parseCsv(csvText);
      const targetIndex = Platform.OS === 'android' ? 1 : 0;
      const target = rows[targetIndex] ?? rows[0];
      const selectedUuid = target.name;
      const oldStatus = target.status;
      const newStatus = oldStatus === 'active' ? 'missing' : 'active';
      const baseVersion = target.__version;
      const observation = { flower_head_count: 7, plant_height_cm: 12.5 };

      await runStep(steps, 'selectEntity', () =>
        latestFormRef.current.setValue('/data/plant', selectedUuid)
      );
      await runStep(steps, 'setStatus', () =>
        latestFormRef.current.setValue('/data/plant_status', newStatus)
      );
      await runStep(steps, 'setFlowerCount', () =>
        latestFormRef.current.setValue('/data/flower_head_count', observation.flower_head_count)
      );
      await runStep(steps, 'setHeight', () =>
        latestFormRef.current.setValue('/data/plant_height_cm', observation.plant_height_cm)
      );
      await runStep(steps, 'refreshSnapshot', () => latestFormRef.current.refreshSnapshot('m4.6.6'));
      await waitForTick();
      await waitForTick();
      ...
      const submitResult = await runStep(steps, 'submitObservation', () =>
      ...
      const checks = {
        ...
        entityUpdateBlock: entity.update === '1',
        entityIdIsSelectedUuid: entity.id === selectedUuid,
        baseVersionFromCsv: entity.baseVersion === baseVersion,
        measurementNotInEntityBlock: !/flower_head_count|plant_height_cm/.test(entity.block),
        statusIsChanged: newStatus !== oldStatus,
      ...
      const summary = {
        ...
        newStatus,
        baseVersion,
        expectedNewVersion: String(Number(baseVersion) + 1),
        observation,
        entity,
        submitResult,
        checks,
      ...
      console.log(
        `M466_RUNTIME_DONE::ok=${summary.ok}::uuid=${selectedUuid}::${oldStatus}->${newStatus}::baseVersion=${baseVersion}::status=${submitResult?.status}::instanceId=${submitResult?.instanceId}`
      );
      console.log(`M466_RUNTIME_RESULT::${JSON.stringify({ summary })}`);
    };

    const watchdog = setTimeout(() => {
      if (cancelled) return;
      console.log('M466_RUNTIME_HANG::' + JSON.stringify({ phase: latestPhaseRef.current }));
    ...
    run().catch((error) => {
      ...
      clearTimeout(watchdog);
      const resolved = error instanceof Error ? error : new Error(String(error));
      console.log(`M466_RUNTIME_CRASH::${resolved.message}`);
...
export default function M466UpdateSlice() {
  ...
  return (
    <XFormsProvider host={host}>
      <ScrollView style={styles.root} contentContainerStyle={styles.container}>
        <Text style={styles.title}>M4.6.6 Entity update (observation) gate</Text>
